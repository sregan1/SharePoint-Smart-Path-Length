...
| **Throttling-aware status** | If SharePoint throttles the background scan, a banner appears above the tree explaining the wait or the reduced concurrency, and any still-scanning library's spinner shows the same explanation on hover — so a scan that's taking a while never just looks stuck |
