...
4. While a library's background check is still running, its folders show a spinner alongside their status icon — the icon itself (an item's own length) is always accurate right away; it's only the dot (anything below it) that isn't final until the spinner goes away. If that spinner seems to be taking a while, hover it — it'll say if SharePoint has asked the tool to slow down (see [When SharePoint Slows the Tool Down](#when-sharepoint-slows-the-tool-down)), and a banner appears above the tree for as long as that's happening. The background check pauses automatically while a page editor is editing the page, and results are kept in your browser for about an hour so repeated visits in the same session don't repeat the check unnecessarily. Hover a library's icon to see whether its current check came from that cache or a live scan, and how long ago.
...
- **In the Explorer**, a banner appears above the tree the moment background scanning is paused or slowed for this reason, and the spinner next to any library still being checked shows the same explanation on hover — so a scan that's taking a while never just looks stuck. The **Activity log** additionally records every pause, slow-down and recovery step, with the reason and the wait SharePoint asked for, if you want the full history rather than just the current state.
